module.exports = (query, context, subdomain, channelId ) => {
  const { data = [], note = "", suggestions = "" } = context;
  ...
  const prompt = `
    ...
    if needed add a message link (do not add too many links) for relevant message use in this format -  <https://${subdomain}.slack.com/archives/${channelId}/p{timestamp_exlcuding_decimal_point} | Message Link>.
